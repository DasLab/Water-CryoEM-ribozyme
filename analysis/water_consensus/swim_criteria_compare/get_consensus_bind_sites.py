...
files = glob('*_all_solvent_consensus_status.csv')
...
for dfile in files:
    ...
    pdbs = {}
    for model in MODEL_RENAME_R.keys():
        pdb = PandasPdb().read_pdb(MODEL_RENAME_R[model])
    ...
    all_bind = pd.read_csv('all_solvent_bindingsite.csv')

    #for model in df['model'].unique():
    mods = [col.split()[-1] for col in all_columns]
    for model in mods:
        print(model)
        ...
        df[f"consensus of mg in {model}"] = df.apply(lambda row: get_consensus(row,model,all_bind,'mg') ,axis=1)
        df[f"consensus of wat in {model}"] = df.apply(lambda row: get_consensus(row,model,all_bind,'wat') ,axis=1)


    df.to_csv(f'{dfile[:-4]}_withconsensus.csv',index=False)

    wat = df[df.solvent=="HOH"]
    mg = df[df.solvent=="MG"]

    wat['Consensus'] = ((wat[f'within 1A of wat in {mods[0]}']!=0) & (wat[f'exact binding spot of wat in {mods[0]}']>0))
    mg['Consensus'] = ((mg[f'within 1A of mg in {mods[0]}']!=0) & (mg[f'exact binding spot of mg in {mods[0]}']>0))
    wat['Consensus2'] = ((wat[f'within 1A of wat in {mods[0]}']!=0) & (wat[f'binding spot of wat in {mods[0]}']>0))
    mg['Consensus2'] = ((mg[f'within 1A of mg in {mods[0]}']!=0) & (mg[f'binding spot of mg in {mods[0]}']>0))
    print(len(wat),wat[f"consensus of wat in {mods[0]}"].sum(),wat[f"Consensus"].sum(),wat[f"Consensus2"].sum())
    print(len(mg),wat[f"consensus of mg in {mods[0]}"].sum(),mg[f"Consensus"].sum(),mg[f"Consensus2"].sum())
